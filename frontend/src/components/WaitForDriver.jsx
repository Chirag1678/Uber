const WaitForDriver = ({vehicleType, fares, pickup, destination, waitForDriver, setWaitForDriver}) => {
    const ride = {
  ...
  if (!waitForDriver) return null;
  
  return (
    <div>
        <div className="relative">
            {waitForDriver && <i className="absolute ri-arrow-down-s-line text-3xl leading-none right-[2.3rem]" onClick={()=>setWaitForDriver(false)}></i>}
            <p className=" text-center text-xl font-semibold border-b pb-4">Looking for nearby drivers</p>
        </div>
